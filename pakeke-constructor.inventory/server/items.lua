...
local itemPartition = partition(5,5)
    ...
    for _, player in ipairs(invPlayers) do
        ...
        if ix then
            for item in itemPartition:foreach(player.x, player.y) do
                ...
                if not seen[item] and d < INTERACTION_DISTANCE and d < best_dist then
                    pickup = item
                end
            end
        end
        if pickup then
            server.broadcast("setInventoryItem", player, ix, iy, pickup)
