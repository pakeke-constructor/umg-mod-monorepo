...
function buffAPI.buffHealth(ent, amount, source_ent, buffdepth)
    local e = server.entities.buff(ent, BUFF_TYPES.HEALTH, amount, source_ent, buffdepth)
    e.color = {0.7,0.7,0.2}
end


function buffAPI.buffDamage(ent, amount, source_ent, buffdepth)
    local e = server.entities.buff(ent, BUFF_TYPES.ATTACK_DAMAGE, amount, source_ent, buffdepth)
    e.color = {0.5,0.9,0.3}
end


function buffAPI.buffAttackSpeed(ent, amount, source_ent, buffdepth)
    local e = server.entities.buff(ent, BUFF_TYPES.ATTACK_SPEED, amount, source_ent, buffdepth)
    e.color = {0.7,0.7,0.2}
...
function buffAPI.buffSpeed(ent, amount, source_ent, buffdepth)
    local e = server.entities.buff(ent, BUFF_TYPES.SPEED, amount, source_ent, buffdepth)
    e.color = {0.2,0.2,0.7}
end
